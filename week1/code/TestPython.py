...
#print(str(result))
#Grabber=opencv_wrapper.InitGrabber()

#thread = threading.Thread(target=run_cpp(PortVoltages,PortNum,ActuationWaitTime,TotalTime,user_quit,voltage_mock,LogSwitchOff))
#thread.start()
#opencv_wrapper.start_actuation(PortVoltages,PortNum,ActuationWaitTime,TotalTime,user_quit,voltage_mock,
#                                LogSwitchOff,ActuationInSitu,OutputInitialPos,ImportNormal,SaveImage)
#TimeNumTest=1000
#for ii in range(0,TimeNumTest):
#    PortVoltages[0]=PortVoltages[0]+0.0001    
#    time.sleep(0.01)
#opencv_wrapper.stop_actuation()
#time.sleep(20)
    ...
    if len(line)==0:
        ...
        if count>1:
            ...
            for ii in range(0,PointNum):        
                PosXAve[ii]=PosXAve[ii]+float(LineNow[44+3*ii])
                PosYAve[ii]=PosYAve[ii]+float(LineNow[44+3*ii+1])
                PosZAve[ii]=PosZAve[ii]+float(LineNow[44+3*ii+2])
##                fout.write('%25.15f %25.15f %25.15f' %(PosXAve[ii],PosYAve[ii],PosZAve[ii]))
##            fout.write('\n')
f.close() 
fout.close()   
fout=open("AveragePos.txt","w", encoding="utf-8")
for ii in range(0,PointNum):
